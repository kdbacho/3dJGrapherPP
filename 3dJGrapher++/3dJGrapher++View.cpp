...
int COpenGLView::OnCreate(LPCREATESTRUCT createStruct)
{
	...
	if (!a)
	{
		MessageBox(_T("ChoosePixelFormat::Error"));
		return -1;
	}
	//Set pixel format
	if (!SetPixelFormat(dcont, a, &pixelDesc))
	{
		MessageBox(_T("SetPixelFormat::Error"));
		return -1;
	}

	//Create OpenGL context
	if (!(cont = wglCreateContext(dcont)))
	{
		MessageBox(_T("wglCreateContext::Error"));
		return -1;
	}

	//Initialize context
	if (!wglMakeCurrent(dcont, cont))
	{
...
void COpenGLView::DefaultGraphic()
{
	...
	*p++ = z_size;
	...
	HANDLE hFile = CreateFile(_T("sine.dat"),
		GENERIC_WRITE,
		0, 0,
		CREATE_ALWAYS,
...
void COpenGLView::OnSize(UINT nType, int cx, int cy)
{
	...
	glLoadIdentity();
	gluPerspective(theta_per, dAspect, 0.1, 10000.);
	glViewport(0, 0, cx, cy);
}

void COpenGLView::OnEditProp()
{
	if (!mdlg)
	{
		mdlg = new CPropDlg(this);
		mdlg->Create(IDD_PROP);
	}
	else
		mdlg->SetActiveWindow();
}